...
$run_disp = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM agency_seat_allocation LEFT JOIN eticket ON agency_seat_allocation.allocation_id = eticket.allocation_id LEFT JOIN flights USING(flight_id) WHERE flight_id='".$id."' ".$str." ORDER BY eticket.createdon DESC");
//echo "SELECT SQL_CALC_FOUND_ROWS * FROM agency_seat_allocation LEFT JOIN eticket ON agency_seat_allocation.allocation_id = eticket.allocation_id LEFT JOIN flights USING(flight_id) WHERE flight_id='".$id."' ".$str." ORDER BY eticket.createdon DESC";
$check =0;
$show_disp= array();
