...
include('../menu/menu_custom_pilgrim.php');
$disp=0;
$db = new Database();
...
// FOR CSV FILES CONTAINING ; 
	...
	//CLASSES ID ARE MATCHED
				...
				if($data[1]!='')
				{
					$db->query("SELECT * FROM custom_eticket WHERE `flight_date` = '".mysql_real_escape_string($data[0])."' AND `flight_no` ='".mysql_real_escape_string($data[1])."'  AND `departure`='".mysql_real_escape_string($data[3])."' AND `destination`='".mysql_real_escape_string($data[4])."' AND `agency_code` ='".mysql_real_escape_string($data[5])."' AND `passport_no`='".mysql_real_escape_string($data[11])."'");
					if($db->total()==0)
					{
						$id = rand(100000,9999999);
						$ticket_id[] = $id ;
						$bp_id = rand(1000000,9999999);			
						
						$db->query("INSERT INTO `custom_eticket` (`id` ,`flight_date` ,`flight_no` ,`class` ,`departure` ,`destination` ,`agency_code` ,`last_name` ,`first_name` ,`title` ,`gender` ,`dob` ,`passport_no` ,`issue_date` ,`country` ,`exp_date`,ticket_id,createdby, bp_ticketid)VALUES (NULL ,'".mysql_real_escape_string($data[0])."', '".mysql_real_escape_string($data[1])."', '".mysql_real_escape_string($data[2])."', '".mysql_real_escape_string($data[3])."', '".mysql_real_escape_string($data[4])."', '".mysql_real_escape_string($data[5])."', '".mysql_real_escape_string($data[6])."', '".mysql_real_escape_string($data[7])."', '".mysql_real_escape_string($data[8])."', '".mysql_real_escape_string($data[9])."', '".mysql_real_escape_string($data[10])."', '".mysql_real_escape_string($data[11])."', '".mysql_real_escape_string($data[12])."', '".mysql_real_escape_string($data[13])."', '".mysql_real_escape_string($data[14])."', '".mysql_real_escape_string($id)."','".$_SESSION['uid']."','".$bp_id."')");
					}					
				}
