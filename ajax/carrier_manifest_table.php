...
if($agency_id !='all')
{
	$str = ' AND agency_seat_allocation.agency_id='.$agency_id;	
}else{

	$str='';
}

$run_disp = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM agency_seat_allocation LEFT JOIN eticket ON agency_seat_allocation.allocation_id = eticket.allocation_id LEFT JOIN flights USING(flight_id) WHERE flight_id='".$id."' ".$str." ORDER BY eticket.createdon DESC LIMIT $start, $nor");
$num_disp = $db->total();
...
<?php if(!isset($_REQUEST['val'])){?><h3><a href="../common/generatemanifestpdf.php?id=<?php echo base64_decode($_REQUEST['id']).'&agency_id='.$agency_id; ?>" target="_blank"><img src="../images/pdf.png" width="32" /></a></h3> <?php }else{ ?> <h3><a href="../common/generatemanifestpdf.php?id=<?php echo $_REQUEST['id'].'&agency_id='.$agency_id; ?>" target="_blank"><img src="../images/pdf.png" width="32" /></a></h3><?php }?>
