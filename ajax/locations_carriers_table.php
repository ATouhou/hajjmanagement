...
$total = $db->query("SELECT * FROM carriers_location WHERE agency_id='".$_SESSION['agency_id']."' ORDER BY location_name DESC");
$total_val = $db->total();
...
$run_disp = $db->query("SELECT * FROM carriers_location WHERE agency_id='".$_SESSION['agency_id']."' ORDER BY location_name DESC LIMIT $start, $nor");
$num_disp = $db->total();
$show_disp = array();
...
<a href="#" title="Status" onclick="changelocationstatus('status',<?php echo $key['location_id']; ?>,'<?php echo $key['status']; ?>');" class="<?php if($key['status']=='Active'){ echo "icon-5 info-tooltip"; }else echo "icon-3 info-tooltip"; ?>"></a></div>
