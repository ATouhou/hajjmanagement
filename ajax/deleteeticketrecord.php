...
$run = $db->query("select id  from eticket WHERE eno='".$eno."' AND id='".$id."'");
if($db->total()>0)
{
	$row = $db->fetch($run);
	$eitcketid = $row['id'];
	...
	//	print_r($row_boardingpass);
		...
		// DELETE THE BOARDING PASS		
			$db->query("DELETE FROM boardingpass WHERE eno ='".$eno."'  AND id='".$id."'");		
	}
	
	//DELETE THE LUGGAGE
		$db->query("DELETE FROM luggage WHERE eticket_id ='".$id."'");
	//DELETE THE E-TICKET
		$db->query("DELETE FROM eticket WHERE eno ='".$eno."'  AND id='".$id."'");	
}
}
/*
