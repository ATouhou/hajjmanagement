...
{
	$data[] = $row;
	$run_query_boardingpass = $db->query("SELECT allocation_id,bno,seat_no FROM boardingpass WHERE allocation_id='".$row['allocation_id']."' AND eno='".$row['eno']."'");
	if($db->total()>0)
	{
...
<?php } ?>    
    ...
    <td> <a href="<?php echo $_SESSION['basemodule'].'eticket.php?id='.base64_encode($data[0]['eno']); ?>">View</a>&nbsp;<?php if($_SESSION['access_right']!=3){ ?> <a href="#"class="icon-2 info-tooltip" title="Delete" onclick="eticketdelete(<?php  echo $data[0]['eno']; ?>); return false;"></a> <?php } ?>  </td>
  </tr>
<?php } ?>  
</table>
