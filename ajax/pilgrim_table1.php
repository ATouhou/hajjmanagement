...
$total = $db->query("SELECT * FROM pilgrims ORDER BY full_name, pilgrim_status");
$total_val = $db->total();
...
$run_disp = $db->query("SELECT * FROM pilgrims ORDER BY full_name, pilgrim_status  LIMIT $start, $nor");
$num_disp = $db->total();
$show_disp = array();
...
-->     </tr>
        ...
        <tr>
          ...
          <td><?php echo getstatename($key['state']);  ?></td>
			  <td><div align="center"><a href="<?php echo ADMIN; ?>newpilgrim.php?pilgrim_id=<?php echo $key['pilgrim_id']; ?>" title="Edit" class="icon-1 info-tooltip"></a>
