...
$total = $db->query("SELECT * FROM state ORDER BY state_name DESC");
$total_val = $db->total();
...
$run_disp = $db->query("SELECT * FROM state ORDER BY state_name DESC LIMIT $start, $nor");
$num_disp = $db->total();
$show_disp = array();
...
-->     </tr>
        ...
        <tr>
          <td><?php echo $key['state_name'];  ?></td>
			  <td><div align="center"><a href="<?php echo ADMIN; ?>add_state.php?state_id=<?php echo $key['state_id']; ?>" title="Edit" class="icon-1 info-tooltip"></a>
<div id="status_<?php echo $key['state_id']; ?>">        
<a href="#" title="Status" onclick="addstate('status',<?php echo $key['state_id']; ?>,'<?php echo $key['status']; ?>');" class="<?php if($key['status']=='Active'){ echo "icon-5 info-tooltip"; }else echo "icon-3 info-tooltip"; ?>"></a></div>
