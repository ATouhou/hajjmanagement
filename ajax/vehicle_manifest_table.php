...
$run_disp = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM vehicle WHERE agency_id='".$_SESSION['agency_id']."' ORDER BY createdon DESC LIMIT $start, $nor");
$num_disp = $db->total();
...
?>
	  <table id="product-table" width="850">
		<thead>
        <tr>
          <th class="table-header-repeat line-left" ><a href="#">Center</a></th>
          <th class="table-header-repeat line-left" ><a href="#">Vehicle No</a></th>
          <th class="table-header-repeat line-left" ><a href="#">Vehicle Type</a></th>
...
-->     </tr>
		...
		?>
        <tr>
          <td><?php echo getmovementcentername($key['center_id']);  ?></td>
          <td><?php echo $key['vehicle_no']; ?></td>
          <td><?php echo $key['vehicle_type'];  ?></td>
          <td><?php echo $key['capacity'];  ?></td>
          <td><?php echo getmovementlocationname($key['source']);  ?></td>
          <td><?php echo getmovementlocationname($key['destination']);  ?></td>
		  <td><div align="center"><a href="<?php echo $_SESSION['basemodule']; ?>editmanifest.php?id=<?php echo base64_encode($key['vehicle_id']);  ?>" title="Edit" class="icon-1 info-tooltip"></a>&nbsp;<a href="<?php echo $_SESSION['basemodule']; ?>createmanifest.php?id=<?php echo base64_encode($key['vehicle_id']);  ?>" title="Add" class="icon-6 info-tooltip"></a>&nbsp;<a href="<?php echo $_SESSION['basemodule']; ?>viewmanifest.php?id=<?php echo base64_encode($key['vehicle_id']);  ?>" title="View" class="icon-5 info-tooltip"></a></td> 
       </tr>
		<?php
		}
		?>
