...
<!-- CODE TO DEVELOP VARIOUS pdf --->
<?php
$total_val = $row_total['num'];
$noofrecords =160;
$nob = ceil($total_val/$noofrecords);
for($c=0;$c<$nob; $c++){
$start = $c*$noofrecords+1;
$end = $start + $noofrecords-1;
if($c==($nob-1)){
$end = $total_val;
}
$bno = $c+1;
echo '<a href="../common/displaysaudimanifestpdf.php?flight_id='.base64_encode($flight_id).'&s='.$start.'&l='.$end.'&totalrecord='.$total_val.'&bno='.$bno.'" style="width:100px; height:26px; background-color:#000; color:#fff; display:block;padding-top:8px;margin:4px; float:left;" target="_blank"><img src="../images/pdf.png" width="24" />('.$start.' - '.$end.')</a>';

//echo '<a href="../common/displaysaudimanifestpdf.php?flight_id='.base64_encode($flight_id).'&s='.$start.'&l='.$end.'" style="width:80px; height:26px; background-color:#000; color:#fff; display:block;padding-top:8px;margin:4px; float:left;" target="_blank"><center>'.$start.' - '.$end.'</center></a>';
...
<div class="clear"></div>
	<table id="product-table" width="850">
    	<tr>
        	<td align="center" style="font-size:20px;"><b>NATIONAL HAJJ COMISSION OF NIGERIA(NAHCON)</b></td>
         </tr>
    	<tr>
        	<td align="center" style="font-size:20px;"><b>ROYALITY PAYMENT SLIP</b></td>
         </tr>
    </table>
    <p>&nbsp;</p>
	<table id="product-table" width="850">
