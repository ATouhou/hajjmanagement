...
//$run_disp = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM agency_seat_allocation LEFT JOIN eticket ON agency_seat_allocation.allocation_id = eticket.allocation_id LEFT JOIN flights USING(flight_id) WHERE flight_id='".$id."' AND agency_seat_allocation.agency_id='".$_SESSION['agency_id']."' ORDER BY eticket.createdon DESC LIMIT $start, $nor");

$run_disp = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM agency_seat_allocation LEFT JOIN eticket ON agency_seat_allocation.allocation_id = eticket.allocation_id LEFT JOIN flights USING(flight_id) WHERE flight_id='".$id."' ORDER BY eticket.createdon DESC LIMIT $start, $nor");
...
?>
	  <table id="product-table" width="850">
        ...
        <tr id="<?php echo $key['eno']; ?>">
          ...
          <td><?php if($_SESSION['access_right']!=3){ ?><a href="#" onclick="deleteRow('<?php echo $key['eno']; ?>'); return false;" title="Delete" class="icon-2 info-tooltip"></a><?php } ?></td>
       </tr>
		<?php  }	?>
		</tbody>
      </table>     
