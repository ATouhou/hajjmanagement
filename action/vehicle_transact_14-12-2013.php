...
{
	case 'add':
			if($db->query("INSERT INTO `vehicle` (`vehicle_id`, `vehicle_no`, `vehicle_type`, `source`, `destination`, `createdon`, `status`, `createdby`, `agency_id`,departure_date,arrival_date,capacity,center_id) VALUES (NULL, '".mysql_real_escape_string($_POST['vehicle_no'])."', '".mysql_real_escape_string($_POST['vehicle_type'])."', '".mysql_real_escape_string($_POST['source'])."', '".mysql_real_escape_string($_POST['destination'])."', CURRENT_TIMESTAMP, 'Active', '".$_SESSION['uid']."', '".$_SESSION['agency_id']."','".$_POST['departure_date']."','".$_POST['arrival_date']."',".$_POST['capacity'].",".$_POST['center'].")")){
