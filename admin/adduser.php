...
{
	if(category_id=='1' || category_id=='4'|| category_id=='7')
	{
...
if(category_id==2 || category_id==5 || category_id==6)
{
	xmlhttp.open("GET","../ajax/getcarriers.php?value="+category_id,true);
}
if(category_id==3)
{
	xmlhttp.open("GET","../ajax/getagency.php?value="+category_id,true);
}
  ...
  <tr>
    ...
    <td></td>
		...
		<tr>
		  <th>Category</th>
		  <td>
          <select name="category" id="category" class="input-medium" onchange="checkcategory(this.value); return false;" >
		    <option value="">Select Category</option>
		    <?php 
		$state_data = array();
		
		foreach($_SESSION['category'] as $val=>$key)
		{	
			$val = '';
			if(isset($_REQUEST['user_id']) && ($key['cid']==$row['cid']))
			{
				$val ='selected="selected"';			
			}
			
			echo '<option value='.$key['cid'].' '.$val.'>'.$key['cname'].'</option>';
		}
?>
		    </select>      
		    </td>
...
<?php if(isset($_REQUEST['user_id'])){ 
if($row['cid']==2){ 
$run_agency = $db->query("SELECT carriers_id,carriers_name FROM carriers WHERE carriers_status='Active'");
echo '<table widh="750"><tr><th>Carriers Name</th><td>';
echo '<select name="agency_id" id="agency_id" class="input-medium">';
while($row_agency = $db->fetch($run_agency))
{
			if(isset($_REQUEST['user_id']) && ($row_agency['carriers_id']==$row['agency_id']))
			{
				$val ='selected="selected"';			
			}else{ $val =''; }
	
    	  	echo '<option value="'.$row_agency['carriers_id'].'"  '.$val.'>'.$row_agency['carriers_name'].'</option>';
}
echo '</select>';
echo '</td></tr>';
echo '<tr><th>Access Right</th><td>';
echo '<select name="access_right" id="access_right" class="input-medium">';
if($row['access_right']==1){ echo '<option value="1" selected="selected">Admin </option>';}else{ echo '<option value="1">Admin </option>'; } 
if($row['access_right']==2){ echo '<option value="2" selected="selected">Supervisor </option>';}else{ echo '<option value="2">Supervisor </option>'; } 
if($row['access_right']==3){ echo '<option value="3" selected="selected">Employee </option>';}else{ echo '<option value="3">Employee </option>'; } 
echo '</select>';
echo '</td></tr>';
echo '</table>';
}  
...
if($row['cid']==3){ 
$run_agency = $db->query("SELECT state_id,state_name FROM state WHERE status='Active'");
echo '<table widh="750"><tr><th>Agency Name</th><td>';
echo '<select name="agency_id" id="agency_id" class="input-medium">';
while($row_agency = $db->fetch($run_agency))
{
			if(isset($_REQUEST['user_id']) && ($row_agency['state_id']==$row['agency_id']))
			{
				$val ='selected="selected"';			
			}else{ $val =''; }
      	echo '<option value="'.$row_agency['state_id'].'"  '.$val.'>'.$row_agency['state_name'].'</option>';
}
echo '</select>';
echo '</td></tr>';
echo '<tr><th>Access Right</th><td>';
