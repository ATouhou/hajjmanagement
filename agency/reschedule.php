...
// CODE TO GET PREVIOUS INFORMATION ABOUT THE E-TICKET
...
$run_pilgrim = $db->query("SELECT pilgrim_id, full_name, passport_no FROM pilgrims WHERE pilgrim_id='".$previous_data['pilgrim_id']."' AND state ='".$previous_data['agency_id']."' AND  status='Active'");
...
$run = $db->query("SELECT pilgrim_id, full_name, passport_no FROM pilgrims WHERE passport_no='".$pilgrim_data['passport_no']."' AND state ='".$_SESSION['agency_id']."' AND  status='Active'");
if($db->total()>0)
{
/*	$row = $db->fetch($run);
...
$sql ="SELECT allocation_id,seat_allocated, flight_no FROM agency_seat_allocation asa LEFT JOIN flights USING(flight_id) WHERE asa.agency_id='".$_SESSION['agency_id']."' AND date1 >='".date('Y-m-d')."'";
...
//$sql = "SELECT allocation_id,seat_allocated, flight_no FROM agency_seat_allocation asa LEFT JOIN flights USING(flight_id) WHERE asa.agency_id='".$_SESSION['agency_id']."' AND date1 >='".date('Y-m-d')."' AND time1>='".$time1."'";
//echo $sql;
